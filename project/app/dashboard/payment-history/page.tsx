...
export default function StudentPaymentHistoryPage() {
  return (
    <RouteGuard requireAuth><Content /></RouteGuard>
  );
}
...
function Content() {
  ...
  const handleExport = () => {
    exportToCsv(payments.map(p => ({
      ...
      amount: p.amount, currency: p.currency, status: p.status, date: p.created_at,
    })), 'payment_history');
  };
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-0">
              <Table>
                <TableHeader><TableRow><TableHead>Reference</TableHead><TableHead>Provider</TableHead><TableHead>Type</TableHead><TableHead>Amount</TableHead><TableHead>Status</TableHead><TableHead>Date</TableHead></TableRow></TableHeader>
                <TableBody>
                  {payments.map(p => (
                    <TableRow key={p.id}>
