...
Deno.serve(async (request) => {
  ...
  const authorizedDocument = authorizationData as AuthorizedDocument | null;
  if (authorizationError || !authorizedDocument) return Response.json({ error: 'Document access denied' }, { status: 403, headers: corsHeaders });
  if (authorizedDocument.bucket_id !== 'vendor-documents' || typeof authorizedDocument.object_path !== 'string' || typeof authorizedDocument.owner_id !== 'string' || !authorizedDocument.object_path.startsWith(`${authorizedDocument.owner_id}/`)) {
    return Response.json({ error: 'Invalid authorized document reference' }, { status: 403, headers: corsHeaders });
  }
  const service = createClient(url, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
  const { data, error } = await service.storage.from('vendor-documents').createSignedUrl(authorizedDocument.object_path, URL_TTL_SECONDS);
